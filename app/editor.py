...
def source_clips(source_id: int):
    ...
    for i, c in enumerate(rows):
        ...
        nxt = rows[i + 1]["start_time"] if i + 1 < len(rows) else None
...
def edit_clip(clip_id: int, edit: ClipEdit):
    ...
    with get_db() as conn:
        row = conn.execute("SELECT * FROM word_clips WHERE id=?", (clip_id,)).fetchone()
        if not row:
        ...
        conn.execute(f"UPDATE word_clips SET {', '.join(sets)} WHERE id=?",
                     (*params, clip_id))
        out = dict(conn.execute("SELECT id, word, start_time, end_time "
                                "FROM word_clips WHERE id=?", (clip_id,)).fetchone())
    _invalidate()
...
def delete_clip(clip_id: int):
    ...
    init_db()
    with get_db() as conn:
        row = conn.execute("SELECT word FROM word_clips WHERE id=?", (clip_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail=f"no clip {clip_id}")
        conn.execute("DELETE FROM word_clips WHERE id=?", (clip_id,))
...
def add_clip(source_id: int, clip: NewClip):
    """Add a word the transcription missed."""
    init_db()
    import re
